...
final class AIProviderConfigTests: XCTestCase {
    func testRoleAPIKeysMapToDedicatedKeychainAccounts() {
        XCTAssertEqual(CredentialStore.Key.aiRoleAPIKey(for: "head"), .aiHeadAPIKey)
        XCTAssertEqual(CredentialStore.Key.aiRoleAPIKey(for: "schedule"), .aiScheduleAPIKey)
        XCTAssertEqual(CredentialStore.Key.aiRoleAPIKey(for: "capacity"), .aiCapacityAPIKey)
        XCTAssertEqual(CredentialStore.Key.aiRoleAPIKey(for: "chat"), .aiChatAPIKey)
        XCTAssertNil(CredentialStore.Key.aiRoleAPIKey(for: "unknown"))
    }
    ...
    func testTestConnectionRequest_ZAIBearerAuth() {
        ...
        XCTAssertEqual(req?.url?.absoluteString, "https://api.z.ai/api/coding/paas/v4/models")
        XCTAssertEqual(req?.value(forHTTPHeaderField: "Authorization"), "Bearer sk-z")
    }

    func testTestConnectionRequest_ZAIUsesCustomEndpoint() {
        let req = AIProviderConfig.testConnectionRequest(
            provider: "zai", apiKey: "sk-z", endpoint: " https://zai.example/v4 "
        )
        XCTAssertEqual(req?.url?.absoluteString, "https://zai.example/v4/models")
    }
